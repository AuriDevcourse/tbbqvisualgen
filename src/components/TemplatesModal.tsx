      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-white/10">
          <h2 className="text-sm font-medium text-white/85">
            <span className="text-[10px] uppercase tracking-[0.18em] text-[#FF6B00] mr-2">▪</span>
            Templates
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-3 border-b border-white/10 flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-md px-3 py-1.5 text-xs text-white placeholder:text-white/30 focus:outline-none focus:border-[#FF6B00]/40"
          />
          <button
            onClick={handleSave}
            disabled={saving}
              ...
              <section className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  ...
                  {onReorderFolders && sortedFolderNames.length > 1 && (
                    <span className="text-[9px] text-white/30">Drag folder name to reorder</span>
                  )}
                </div>
                {sortedEntries.map(([groupLabel, items]) => (
                  <div key={groupLabel} className="flex flex-col gap-1.5">
                    ...
                    >
                      {onReorderFolders && (
                        <GripVertical className="w-3 h-3 text-white/25 hover:text-white/60" />
                      )}
                        ...
                        <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                          {isEditingThis ? (
                            <input
                              ...
                              className="w-full bg-white/10 border border-[#FF6B00]/40 rounded px-1.5 py-0.5 text-[11px] text-white focus:outline-none"
                            />
                          ) : onRenamePreset ? (
                            <button
                            ...
                            >
                              <p className="text-[11px] text-white/85 truncate flex items-center gap-1">
                                {displayName}
                                <Pencil className="w-2.5 h-2.5 text-white/30 opacity-0 group-hover/name:opacity-100 transition-opacity shrink-0" />
                              </p>
                              ...
                              <>
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-1 px-1 py-1">
                                    <Plus className="w-3 h-3 text-white/40 shrink-0" />
                                    <input
                                      type="text"
                                      ...
                                      maxLength={40}
                                      autoFocus
                                      className="flex-1 min-w-0 bg-transparent text-[11px] text-white placeholder:text-white/30 focus:outline-none"
                                    />
                                  </div>
                                </div>
                              </>
                        ...
                        {onHidePreset && !isEditingThis && (
                          confirming ? (
                            ...
                            <button
                              ...
                              title="Hide this preset"
                              className="p-1 rounded text-white/40 hover:text-[#FF0028] hover:bg-[#FF0028]/10 transition-colors"
                            >
                    ...
                    <div className="flex-1 min-w-0">
                      {editingId === t.id ? (
                        ...
                        >
                          ...
                          <p className="text-[9px] text-white/40">{new Date(t.createdAt).toLocaleDateString()}</p>
                        </button>
                      )}
                    </div>
                    {confirming ? (
